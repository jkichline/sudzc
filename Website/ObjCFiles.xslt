...
@end
	...
	- (SoapRequest*) <xsl:value-of select="$name"/>: (id &lt;SoapDelegate&gt;) handler<xsl:apply-templates select="$element" mode="param_selectors"/>;
...
@class <xsl:value-of select="$shortns"/><xsl:value-of select="@name"/>;</xsl:if></xsl:template>
	...
	<xsl:template match="s:complexType" mode="import"><xsl:if test="generate-id(.) = generate-id(key('className', @name)[1])">
#import "<xsl:value-of select="$shortns"/><xsl:value-of select="@name"/>.h"</xsl:if></xsl:template>
...
@class <xsl:value-of select="$shortns"/><xsl:value-of select="substring-after(@type, ':')"/>;</xsl:if></xsl:template>
	...
	<xsl:template match="s:element" mode="import">
		<xsl:variable name="type">
			<xsl:call-template name="getType">
				<xsl:with-param name="value" select="@type"/>
			</xsl:call-template>
...
#import "<xsl:value-of select="$shortns"/><xsl:value-of select="substring-after(@type, ':')"/>.h"</xsl:if></xsl:template>
	...
	<xsl:template match="s:complexType" mode="interface">
		<xsl:if test="generate-id(.) = generate-id(key('className', @name)[1])">
			<xsl:choose>
				<xsl:when test="s:sequence/s:element[@maxOccurs = 'unbounded'] or s:restriction/s:attribute[@wsdl:arrayType]">
	...
	<xsl:template match="s:complexType" mode="interface_object">
		...
		<file>
			<xsl:attribute name="filename"><xsl:value-of select="$shortns"/><xsl:value-of select="@name"/>.h</xsl:attribute>/*
...
@end</file>
	...
	<xsl:template match="s:complexType" mode="implementation_object"><xsl:if test="generate-id(.) = generate-id(key('className', @name)[1])">
		<file>
			<xsl:attribute name="filename"><xsl:value-of select="$shortns"/><xsl:value-of select="@name"/>.m</xsl:attribute>/*
	<xsl:value-of select="$shortns"/><xsl:value-of select="@name"/>.h
